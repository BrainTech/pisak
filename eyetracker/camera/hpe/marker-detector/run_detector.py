...
from marker_detector.square import Square
from utils import rgb2gray, correct_rectangle
from marker_detector.marker import Marker
import debug as db
...
class SquareDetector(object):
    ...
    def set_scale(self, scale):
        self.img, self.draw_img, self.tmp_img, \
            self.canny_img, self.bw_img, self.gray_img = self.scaled_images[
                scale]
        self.scale = scale
...
#        cv.Smooth(img, self.imgs[0],cv.CV_GAUSSIAN,5)
    ...
    def check_recognized(self, mark, conts, i):
        ...
        if self.markers.has_key(ident):
            ...
            if m.check_conts_with_hint(conts, (ident, hint), self.scale_factor):
                return m
...
if __name__ == "__main__":
    ...
    while cv.WaitKey(1) != 27:
        img, t = i_p.getNext()
        markers = m_d.find_markers(img, t)
        #print t
        if len(markers) > 0:
            m = markers[0]
            #print 'points', m.points
            print '{};{}|{};{}|{};{}|{};{}'.format(
                m.points[0][0], m.points[0][1], 
                m.points[1][0], m.points[1][1], 
                m.points[2][0], m.points[2][1],
                m.points[3][0], m.points[3][1]
            )
            #print 'corners', m.corners
            #print 'object_points', m.object_points
        else:
            print 'none'
        sys.stdout.flush()
